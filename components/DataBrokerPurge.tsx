...
const DataBrokerPurge = () => {
  ...
  const handleNukeClick = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      setMessage('Your personal information has been removed from data brokers.');
    }, 2000);
  };
  ...
  const handleKnoxClick = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      setMessage('Your personal information will be kept on data broker sites.');
    }, 2000);
  };

  return (
    <div className="max-w-[1400px] mx-auto space-y-8 animate-in slide-in-from-bottom-8 duration-700">
      <h1 className="text-5xl font-black uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-agape-sovereign-light to-agape-sovereign-dark">Data Broker Purge</h1>
      <p className="text-universal-irony">Remove your personal information from data broker websites.</p>
      {loading ? (
        <LoadingSpinner />
      ) : message ? (
