...
const FootprintFinder = () => {
  ...
  const handleNukeClick = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      setMessage('All accounts and data have been deleted.');
    }, 2000);
  };
  ...
  const handleKnoxClick = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      setMessage('All accounts and data will be kept.');
    }, 2000);
  };

  return (
    <div className="max-w-[1400px] mx-auto space-y-8 animate-in slide-in-from-bottom-8 duration-700">
      <h1 className="text-5xl font-black uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-agape-sovereign-light to-agape-sovereign-dark">Footprint Finder</h1>
      <p className="text-universal-irony">Discover your digital footprint and see where your data is being used.</p>
      {loading ? (
        <LoadingSpinner />
      ) : message ? (
