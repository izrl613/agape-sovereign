...
### Version: 2026-LTS | Compliance: ECRA 2026 | Runtime: Firebase + Local Gemma AI
...
- **AI Backend:** Local Gemma AI (via Ollama)
...
var OLLAMA_BASE_URL = process.env.OLLAMA_BASE_URL || process.env.VITE_OLLAMA_BASE_URL || "http://localhost:11434";
var DEFAULT_MODEL = process.env.ARCHITECT_MODEL || process.env.VITE_ARCHITECT_MODEL || "gemma4:e4b";
...
async function startServer() {
  ...
  const EXPECTED_ORIGIN = process.env.WEBAUTHN_ORIGIN || (process.env.NODE_ENV === "production" ? "https://sovereign.nyc" : `http://localhost:${Number(process.env.PORT) || 5e3}`);
  app.post("/api/auth/register-options", async (req, res) => {
    try {
      ...
      const existingSnap = await db.collection("passkey_credentials").where("email", "==", resolvedEmail).get();
  ...
  app.post("/api/auth/login-options", async (req, res) => {
    try {
      ...
      if (!email) return res.status(400).json({ error: "email is required" });
      const credsSnap = await db.collection("passkey_credentials").where("email", "==", email).get();
      if (credsSnap.empty) {
