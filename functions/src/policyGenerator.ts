...
export const generatePolicyDocument = onRequest(
  ...
  async (req, res) => {
    try {
      ...
      if (!docType || !["privacy", "terms"].includes(docType)) { // eslint-disable-line max-len
        const msg = "Invalid document type. Must be 'privacy' or 'terms'.";
      ...
      const sha256Id = crypto.createHash("sha256")
      ...
      doc.setFontSize(12);
      const msgBody =
        "This document is dynamically generated and " +
        "cryptographically verifiable.";
      doc.text(msgBody, 20, 40);
      ...
      const filename = `Agape_Sovereign_${title.replace(/\s+/g, "_")}.pdf`;
      res.setHeader(
