...
interface ProfilePanelProps {
  user: { name: string, email: string, provider: string, photoURL?: string } | null;
  onClose: () => void;
}

export const ProfilePanel: React.FC<ProfilePanelProps> = ({ user, onClose }) => {
  const [email, setEmail] = useState("");
  const [savedEmails, setSavedEmails] = useState([user?.email || "user@agape.nyc"]);
      ...
      <GlassCard className="w-[340px] mt-14 mr-4 p-5" style={{ animation: "slide-in-left 0.3s ease" }} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-3 p-3 rounded-lg mb-4 border" style={{ background: "rgba(255,46,159,0.06)", borderColor: "rgba(255,46,159,0.15)" }}>
          ...
          <div>
            ...
            <div className="font-['Share_Tech_Mono'] text-[0.58rem] mt-0.5" style={{ color: NEON.blue }}>● {user?.provider} authenticated</div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          {[
            { icon: "🔑", label: "Passkey Settings", color: NEON.blue, action: () => {} }, 
            { icon: "☁️", label: "Backup to Google Account", color: NEON.blue, action: () => {} }, 
            { icon: "🍎", label: "Backup to Apple Account", color: NEON.textMuted, action: () => {} }, 
            { icon: "🔓", label: "Sign Out", color: NEON.magenta, action: handleSignOut }
          ].map(({ icon, label, color, action }) => (
            <button key={label} onClick={action} className="flex items-center gap-2.5 py-2 px-2.5 bg-transparent rounded-md font-['Rajdhani'] text-[0.8rem] cursor-pointer text-left border" style={{ borderColor: `${color}22`, color }}>
