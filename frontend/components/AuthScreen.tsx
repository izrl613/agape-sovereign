      ...
      <GlassCard className="w-[480px] p-12 text-center" style={{ animation: "slide-in-up 0.6s ease" }}>
        ...
        {step === "landing" && !scanning && (
          <div style={{ animation: "fade-in 0.4s ease" }}>
            ...
            <div className="text-[0.78rem] mb-5 leading-relaxed" style={{ color: NEON.textMuted }}>
            ...
            <div className="mb-4 text-left">
              <input 
                ...
                className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-[#00D4FF]/50 transition-all font-mono text-sm"
             ...
             <div className="flex flex-col gap-3">
              ...
              <button className="btn-neon neon-border py-3 px-5 rounded-lg text-white font-semibold flex items-center justify-center gap-3 text-[0.95rem]" onClick={() => {
                const el = document.getElementById("passkey-email-input") as HTMLInputElement;
                const emailVal = el?.value || "user@agape.nyc";
                setScanning(true);
                setTimeout(() => {
                  setScanning(false);
                  setStep("passkey");
