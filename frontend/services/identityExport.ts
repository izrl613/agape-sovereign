...
/**
 * Encrypt payload with AES-256-GCM using Web Crypto API
 */
export async function encryptWithPasskey(plaintext: string, passkeyCredentialId: string): Promise<string> {
  // In production, use the passkey's private key to derive encryption key
  // For now, we'll use a derived key from the credential ID + user entropy
  ...
  const keyMaterial = await crypto.subtle.importKey(
    'raw',
    new TextEncoder().encode(passkeyCredentialId),
    { name: 'PBKDF2' },
    false,
  ...
  const key = await crypto.subtle.deriveKey(
    {
      name: 'PBKDF2',
      salt: new TextEncoder().encode('agape-sovereign-export'),
      iterations: 100000,
      hash: 'SHA-256',
    },
    keyMaterial,
...
/**
 * Decrypt payload with Passkey
 */
export async function decryptWithPasskey(ciphertext: string, passkeyCredentialId: string): Promise<string> {
  const encoder = new TextEncoder();
  ...
  const keyMaterial = await crypto.subtle.importKey(
    'raw',
    encoder.encode(passkeyCredentialId),
    { name: 'PBKDF2' },
    false,
    ['deriveBits', 'deriveKey']
  );
  ...
  const key = await crypto.subtle.deriveKey(
    {
      name: 'PBKDF2',
      salt: encoder.encode('agape-sovereign-export'),
      iterations: 100000,
      hash: 'SHA-256',
    },
    keyMaterial,
...
/**
 * Generate BIP39 mnemonic phrase for recovery
 */
export function generateMnemonic(): string {
  // Use a simplified wordlist for demo - in production use bip39 library
  const wordlist = [
    ...
    'family', 'famous', 'fan', 'fancy', 'farm', 'fashion', 'fat', 'fatal', 'father', 'fatigue', 'fault',
    ...
    'garlic', 'gas', 'gasp', 'gate', 'gather', 'gauge', 'gaze', 'general', 'genius', 'genre', 'gentle',
    ...
    'mandate', 'mango', 'mansion', 'manual', 'maple', 'marathon', 'margin', 'marine', 'market',
    ...
    'picture', 'piece', 'pig', 'pigeon', 'pill', 'pilot', 'pink', 'pioneer', 'pipe', 'pistol', 'pit',
    'pizza', 'place', 'planet', 'plastic', 'plate', 'play', 'please', 'pledge', 'pluck', 'plug',
    'plunge', 'poem', 'poet', 'point', 'polar', 'pole', 'police', 'pond', 'pony', 'pool', 'popular',
    'portion', 'position', 'possible', 'post', 'potato', 'pottery', 'poverty', 'powder', 'power',
    'practice', 'praise', 'predict', 'prefer', 'prepare', 'present', 'pretty', 'prevent', 'price',
    'pride', 'primary', 'print', 'priority', 'prison', 'private', 'prize', 'problem', 'process',
    ...
    'rapid', 'rare', 'rate', 'rather', 'raven', 'raw', 'reach', 'react', 'read', 'real', 'realize',
    'really', 'reason', 'rebel', 'rebuild', 'recall', 'receive', 'recipe', 'record', 'recycle', 'reduce',
    ...
    'roof', 'rookie', 'room', 'rose', 'rotate', 'rough', 'round', 'route', 'royal', 'rubber', 'rude',
    ...
    'truck', 'true', 'truly', 'trumpet', 'trust', 'truth', 'try', 'tube', 'tuition', 'tumble', 'tuna',
    ...
    'void', 'volcano', 'volume', 'vote', 'voyage', 'wage', 'wagon', 'wait', 'walk', 'wall', 'walnut',
    'want', 'warfare', 'warm', 'warrior', 'wash', 'wasp', 'waste', 'water', 'wave', 'way', 'wealth',
...
/**
 * Export user's identity blueprint
 */
export async function exportIdentityBlueprint(
  passkeyCredentialId: string
  ...
  // Fetch all IVM data
  const snap = await getDocs(collection(db, 'users', userId, 'ivmData'));
  ...
  // Encrypt the blueprint
  const plaintext = JSON.stringify(blueprint, null, 2);
  const encryptedPayload = await encryptWithPasskey(plaintext, passkeyCredentialId);

  // Recovery instructions
  const recoveryInstructions = `
AGAPE SOVEREIGN — IDENTITY RECOVERY INSTRUCTIONS
=================================================

This package contains your encrypted Digital Identity Blueprint.
...
3. Authenticate with your Passkey (biometric/WebAuthn)
4. Enter your 12-word mnemonic phrase when prompted
4. The app will decrypt and restore your complete identity

YOUR MNEMONIC (STORE SECURELY - NEVER SHARE):
${mnemonic}
...
⚠️  WARNING: This mnemonic provides full access to your identity.
    Store it offline. Write it on paper. Use a hardware password manager.
    Do NOT store digitally (screenshots, cloud notes, email).
    If lost, your identity CANNOT be recovered.

AGAPE SOVEREIGN — ONE LOVE
`;
...
/**
 * Recover identity from encrypted payload
 */
export async function recoverIdentity(
  encryptedPayload: string,
  passkeyCredentialId: string,
  mnemonic: string
): Promise<IdentityBlueprint | null> {
  try {
    const decrypted = await decryptWithPasskey(encryptedPayload, passkeyCredentialId);
    const blueprint = JSON.parse(decrypted) as IdentityBlueprint;

    // Verify mnemonic (in production, verify against stored hash)
    // For now, just verify the seal
    const seal = generateSHA256Seal({ ...blueprint, sha256Seal: '' });
    if (seal !== blueprint.sha256Seal) {
...
/**
 * Store blueprint in Google Drive / Firebase Storage (user's choice)
 */
export async function storeBlueprintToCloud(
  blueprint: IdentityBlueprint,
  destination: 'google' | 'firebase' | 'local'
): Promise<string> {
  const payload = JSON.stringify(blueprint, null, 2);
  
  switch (destination) {
    case 'firebase':
      // Store in user's Firestore private collection
      await setDoc(doc(db, 'users', blueprint.userId, 'exports', `blueprint_${Date.now()}`), {
        blueprint,
        encrypted: true,
      });
      return 'firebase';
    ...
    default:
      ...
      a.href = url;
      a.download = `agape-sovereign-blueprint-${blueprint.userId}-${Date.now()}.json`;
      a.click();
