...
export const AdminPortal = () => {
    ...
    const fetchStats = async () => {
      try {
        ...
        for (const userDoc of usersSnap2.docs) {
          const scansSnap = await getDocs(collection(db, 'users', userDoc.id, 'diff_scans'));
  ...
  const verifyAdminPasskey = async () => {
    setIsVerifying(true);
    setVerificationError('');
    setTerminalLogs([]);
    
    const logs = [
      `[SYS] Initializing Enclave WebAuthn L3 Assertion...`,
      `[SYS] Operator bound: ${userEmail}`,
      `[SYS] Establishing secure handshake with Firebase Auth...`,
      `[SYS] Generating cryptographic challenge token...`
    ];
    ...
    try {
      ...
      setTerminalLogs(prev => [...prev, `[SYS] Received WebAuthn options from enclave API.`]);

      const assertionResponse = await startAuthentication(options);
      setTerminalLogs(prev => [...prev, `[SYS] Assertion response generated by browser.`]);
      ...
      if (verified) {
        setTerminalLogs(prev => [...prev, `[SYS] Cryptographic challenge matches. ROOT SESSION UNLOCKED.`]);
        setIsPasskeyVerified(true);
        toast.success("Administrative enclave unlocked successfully.");
      } else {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center p-8 relative">
        <div className="max-w-2xl w-full bg-[#0B1020]/90 border border-white/10 rounded-2xl p-8 backdrop-blur-xl relative overflow-hidden shadow-2xl">
          ...
          <NeonText color={NEON.blue} size="2rem" weight={900} className="tracking-tight uppercase">
            ADMIN ENCLAVE SIGNATURE ASSERTION
          </NeonText>
          
          <p className="text-slate-300 font-mono text-sm leading-relaxed mt-4 mb-6 max-w-md mx-auto">
            Hardware-bound biometric verification is active. Please authenticate using the registered WebAuthn universal passkey for:
            <span className="text-[#00D4FF] block mt-1 font-bold">{userEmail}</span>
          </p>

          <div className="w-full bg-black/80 border border-white/5 rounded-xl p-4 font-mono text-[10px] text-left text-[#00D4FF] h-40 overflow-y-auto mb-6 space-y-1">
            <div className="text-slate-500">// Sovereign Administrative Console -- Security Level 3 //</div>
            {terminalLogs.map((log, index) => {
              let color = 'text-[#00D4FF]';
              if (log.startsWith('[ERR]')) color = 'text-[#FF2E9F]';
              if (log.startsWith('[WAR]')) color = 'text-[#FF7A18]';
              return (
            ...
            {isVerifying && (
              <div className="text-slate-400 animate-pulse">
                [SYS] Awaiting response... <span className="inline-block w-1.5 h-3 bg-slate-400 animate-ping" />
              </div>
            )}
            {terminalLogs.length === 0 && !isVerifying && (
              <div className="text-slate-500">Awaiting user initiation...</div>
            )}
          </div>

          {verificationError && (
            <p className="text-xs text-[#FF2E9F] font-mono mb-4">{verificationError}</p>
          )}
          ...
          <div className="flex flex-col items-center gap-3">
            <NeonButton 
              ...
              className="w-full max-w-xs shadow-[0_0_15px_rgba(0,212,255,0.2)]"
            >
              {isVerifying ? 'ASSERTING IDENTITY...' : 'VERIFY ADMIN PASSKEY'}
            </NeonButton>
          </div>
        </div>
      </div>
